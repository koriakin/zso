...
static void scull_access_setup (dev_t devno, struct scull_adev_info *devinfo)
{
	...
	dev->dev = device_create(scull_class, 0, dev->devno, 0, "%s", devinfo->name);
	if (IS_ERR(dev->dev)) {
		printk(KERN_NOTICE "Error %ld adding %s to sysfs", PTR_ERR(dev->dev), devinfo->name);
		dev->dev = 0;
	}
}
...
void scull_access_cleanup(void)
{
	...
	for (i = 0; i < SCULL_N_ADEVS; i++) {
		...
		device_destroy(scull_class, dev->devno);
		cdev_del(&dev->cdev);
