...
struct scull_pipe {
	...
	dev_t devno;			   /* Our device major+minor */
        struct cdev cdev;                  /* Char device structure */
	struct device *dev;		   /* Sysfs device structure */
};
...
static void scull_p_setup_cdev(struct scull_pipe *dev, int index)
{
	...
	dev->dev = device_create(scull_class, 0, dev->devno, 0, "scullpipe%d", index);
	if (IS_ERR(dev->dev)) {
		printk(KERN_NOTICE "Error %ld adding scullpipe%d to sysfs", PTR_ERR(dev->dev), index);
		dev->dev = 0;
	}
}
...
void scull_p_cleanup(void)
{
	...
	for (i = 0; i < scull_p_nr_devs; i++) {
		device_destroy(scull_class, scull_p_devices[i].devno);
